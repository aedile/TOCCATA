...
#define einline static inline __attribute__((always_inline))
...
/*
 * Gyruss's sub-CPU is a KONAMI-1: a 6809 with its opcodes scrambled. Only the opcode byte is
 * encrypted - operands and data are read plain - so the fetch is split in two here and the
 * board supplies whatever decryption it needs. Left NULL, this is an ordinary 6809.
 */
unsigned char (*e6809_read8_op) (unsigned address);
void (*e6809_write8) (unsigned address, unsigned char data);

/* obtain a particular condition code. returns 0 or 1. */

einline unsigned get_cc (unsigned flag)
{
...
einline unsigned test_c (unsigned i0, unsigned i1,
...
einline unsigned test_n (unsigned r)
{
	return (r >> 7) & 1;
}
...
einline unsigned get_reg_d (void)
{
...
{
#ifdef E6809_READ8
	return (unsigned) E6809_READ8 (address);
#else
	return (unsigned) (*e6809_read8) (address);
#endif
}
...
/* the same, but for the byte that is the instruction itself */

einline unsigned pc_read8_op (void)
{
	unsigned data;

	if (e6809_read8_op)
		data = (unsigned) (*e6809_read8_op) (reg_pc);
	else
		data = read8 (reg_pc);
	reg_pc++;
...
einline unsigned ea_direct (void)
{
...
einline unsigned ea_extended (void)
{
	return pc_read16 ();
}
...
static inline __attribute__((always_inline)) unsigned sstep_inl (unsigned irq_i, unsigned irq_f)
{
	unsigned op;
	unsigned cycles = 0;
	unsigned ea, i0, i1, r;
...
/* --- TRENCHRUNNER addition: expose the program counter for diagnostics --- */
...
{
	return sstep_inl (irq_i, irq_f);
}

#ifdef E6809_IRQ_LINE
/* Run until at least `cycles` cycles have elapsed, sampling the IRQ line
 * (a variable supplied by the including file) before every instruction. */
unsigned e6809_run (unsigned cycles)
{
	unsigned total = 0;
	while (total < cycles) {
#ifdef E6809_PRE_STEP
		E6809_PRE_STEP ();
#endif
